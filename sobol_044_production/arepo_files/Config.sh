...
FOF_SECONDARY_LINK_TYPES=1+4+8+16+32   # 2^type for the types linked to nearest primaries
...
DL_MULTISPECIES=2                      #turn on the option to track multiple grain species; also specify the number of species
#DL_SUBLIMATION                         #loss of dust mass due to sublimation in the presence of ISRF; DL_MULTISPECIES is required
#DL_NODRAG                              #do not couple dust to gas through a drag force
DL_GRAIN_BINS=16                       #track grain size distribution information for dust particles, using the specified number of bins; requires cooling, star formation
...
DL_GROWTH_COULOMB                      #enable Coulomb enhancement effect
DL_SPUTTERING                          #loss of dust mass due to thermal sputtering
DL_SPECIES_DEPENDENT_SPUTTERING
...
#DL_PRODUCTION_KICKPOS
#DL_ASTRATION                           #astration of dust due to newborn stars
DL_REFINEMENT                          #refinement of large dust particles
DL_DEREFINEMENT=1                        #derefinement of small dust particles, 0=derefine into closest dust particle with mass above derefinement mass, 1=derefine into closest dust particle
...
#DL_OUTPUT_GASDUST                     #compute and output kernel-weighted dust properties for gas cells
DL_ONLY_HIGHRES_DUST
DL_CLUMPING                           #if set, then we assume a subresolution clumping for both growth and sputtering, where the clumping factor is set by #DL_CLUMPING_FAC and the fraction of dense gas is set by #
...
#SMUGGLE_SFR_DYNTIME_LIMITER                #limits hydro time step to the cell gravity dynamical time
...
SMUGGLE_STAR_FEEDBACK_LIMIT_GAS_TIMESTEP   #turns on time step limiter for gas particles close to active star particles
...
#SMUGGLE_CONSTANT_SNII_RATE                 #using a  constant (rather than evolving) SNII rate per unit mass for SMUGGLE feedback
SMUGGLE_AGB_WINDS                          #returns momentum and energy to the ISM accounting OB and AGB stellar winds
SMUGGLE_STOCHASTIC_HII_PHOTOIONIZATION=1   #stochastic photoionization based on ionizing photon budget (0=each time step; 1=one ionization event per star particle)
...
SMUGGLE_PHOTOIONIZATION_FEEDBACK
#SMUGGLE_FEEDBACK_TEST                      #idealized test for stellar feedback (a star particle in the center of a uniform box)
#SMUGGLE_BOOST_DU
SMUGGLE_NONEQ_ELECTRON_FRACTION
SMUGGLE_PHOTOELECTRIC_HEATING_REMYRUYER_DUST_SCALING
#-------------------------------------- Conduction
